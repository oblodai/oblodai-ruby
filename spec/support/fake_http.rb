...
# It implements the whole seam the SDK needs (`call(request, timeout:)`), so the unit and
# contract suites exercise the real signing, envelope, retry and pagination code with no sockets.
class FakeHTTP
  Recorded = Struct.new(:verb, :url, :headers, :body, :timeout, :max_bytes, keyword_init: true) do
  ...
  # @param script [Array<Hash>] each entry: status:, body:, headers:, raises:, delay: (seconds), url:
  ...
  def call(request, timeout:)
    @calls << Recorded.new(
      verb: request.method, url: request.url, body: request.body, timeout: timeout,
    ...
    if step[:delay]
      if step[:delay] > timeout
        raise Oblodai::TransportError.new("transport.timeout", "request timed out after #{timeout} s")
      end
  ...
  # @return [Hash] a success envelope step answering `operation_id` with a minimal valid body
  def self.ok_for(operation_id, overrides = {})
    ok(Samples.result(operation_id, overrides))
  end
