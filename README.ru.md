...
**API keys**: public id `oblodai_<hex>` и секрет `oblodai_live_<hex>`. Им подписывается любой
подписываемый маршрут — счета, выплаты, возвраты, ссылки, сплиты, кошельки, настройки, документы,
песочница. Выбирать ключ на вызов не нужно, и второй пары, которую пришлось бы держать в
согласии, тоже нет.

Пара песочницы приходит из онбординга песочницы (`merchants.create_sandbox` или dev-магазин в
кабинете): public id `test_oblodai_<hex>` и секрет `oblodai_test_<hex>`, работающие с бесцепочечной
копией шлюза. **Админ-токен онбординга** — совсем другая вещь: он принадлежит оператору шлюза и
открывает только заведение мерчантов (`merchants.create`, `merchants.create_sandbox`), которое идёт
без подписи.
...
Любая опция умеет брать значение из окружения, так что те же две переменные настраивают
развёртывание без правки кода, а `admin_token:` (или `OBLODAI_ADMIN_TOKEN`) добавляет маршруты
заведения мерчантов, если шлюз вы держите сами.

> **Старые раздельные ключи.** У мерчантов, заведённых до перехода на один ключ, ещё может лежать
> старая пара `oblodai_pk_<hex>` (платёжный) / `oblodai_wk_<hex>` (выплатной). Только из-за неё шлюз
> ещё умеет отвечать 403 `merchant.wrong_key_kind`; текущий ключ `oblodai_<hex>` этого кода не
> видит никогда. Передавайте тот из старой пары, который подходит вызову, или попросите поддержку
> перевести вас на один ключ.
...
Вывод денег идёт по тому же ключу:
...
- `sandbox.faucet` начисляет тестовые деньги, потолок — 1000000 за вызов. Передайте
...
`idempotency_key:`, `timeout_ms:` и `deadline_ms:`; опечатка в имени опции
...
Каталог кодов — `Oblodai::Enums::ERROR_CODES`: все 469 кодов ошибок, которыми может ответить шлюз,
...
`invoice.not_payable`, `payment.not_found`, `merchant.bad_signature`,
...
- **На вызов:** `idempotency_key:`, `timeout_ms:`, `deadline_ms:`.
...
| Опция                           | Что делает                                                                     |
| ------------------------------- | -------------------------------------------------------------------------------- |
| `public_id:` / `secret:`        | API-ключ мерчанта; им подписывается любой подписываемый маршрут                   |
...
списка), схемы DTO запросов с английскими описаниями полей, все словари и все 469 кодов ошибок,
