...
#
#   OBLODAI_PUBLIC_ID=test_… OBLODAI_SECRET=… ruby examples/accept_payment.rb
...
endpoint = client.webhooks.register("https://shop.example/oblodai/webhook")
...
current = client.payments.info(order_id: invoice.order_id)
puts "status now: #{current.status} (paid: #{current.paid?}, final: #{current.final?})"
...
if current.underpaid?
  resolution = client.refunds.resolve(uuid: current.uuid, action: "accept")
  puts "resolved: #{resolution.to_h}"
end

# The last 10 invoices, newest first. `each` would walk every page.
client.payments.history(limit: 10).first_page.items.each do |payment|
